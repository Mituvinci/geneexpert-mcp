#!/usr/bin/env python3
"""
Generate Per-Dataset Performance Plots

Shows how each system performs on individual datasets.
Perfect for showing system robustness across different data types:
- Clean datasets (GSE52778, GSE114845, GSE113754)
- Batch effect datasets (GSE141496, GSE47774)
- Lab data (GSE193658)
- Contamination (CONTAM70)

Usage:
    python3 bin/generate_per_dataset_plots.py bulk_evaluation_final_per_experiment.csv [output_prefix]
"""
...
DATASET_LABELS = {
    '1_GSE52778_pe_clean': 'GSE52778\n(Clean)',
    '2_GSE114845_se_clean': 'GSE114845\n(Clean)',
    '3_GSE113754_pe_clean': 'GSE113754\n(Clean)',
    '4_GSE141496_batch_effect': 'GSE141496\n(Batch)',
    '5_GSE47774_batch_effect': 'GSE47774\n(Batch)',
    '6_GSE193658_Lab_data': 'GSE193658\n(Lab)',
    '7_GSE114845_se_clean_CONTAM70_with_E.coli_GSE48151': 'CONTAM70\n(70% E.coli)'
}
...
def parse_system_config(system_code):
    ...
    # Default configuration: GPT=stats, Claude=pipeline, Gemini=biology
...
def plot_accuracy_per_dataset(df, systems, output_prefix):
    ...
    ax.set_title('Per-Dataset Decision Accuracy', fontsize=15, fontweight='bold')
    ax.set_xticks(x)
...
def plot_completion_per_dataset(df, systems, output_prefix):
    ...
    for i, system in enumerate(systems):
        ...
        for dataset in datasets:
            subset = df[(df['System'] == system) & (df['Dataset'] == dataset)]
            if len(subset) > 0:
                # Completion = Completed column (already 1/0)
                completed = subset['Completed'].values[0]
    ...
    ax.set_title('Per-Dataset Pipeline Completion Rate', fontsize=15, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([DATASET_LABELS.get(d, d) for d in datasets], fontsize=9)
    ax.set_ylim(0, 105)
    ax.legend(loc='upper left', bbox_to_anchor=(1.01, 1), ncol=1, fontsize=8)
...
def plot_heatmap_accuracy(df, systems, output_prefix):
    ...
    for system in systems:
        ...
        # Only filter systems with more than 1 dataset at 0% (allows contaminated datasets)
        if zero_count <= 1:
            matrix.append(row)
    ...
    ax.set_title('Decision Accuracy Heatmap: Systems × Datasets', fontsize=14, fontweight='bold', pad=20)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 bin/generate_per_dataset_plots.py bulk_evaluation_final_per_experiment.csv [output_prefix]")
        sys.exit(1)

    csv_file = sys.argv[1]
    output_prefix = sys.argv[2] if len(sys.argv) > 2 else 'experiments/bulk_rna_csv_figures/per_dataset_figure'
    ...
    print("\n" + "="*80)
    print("GENERATING PER-DATASET PERFORMANCE PLOTS")
    print("="*80 + "\n")
    ...
    print("Perfect for demonstrating robustness across clean/batch/contaminated data!\n")
