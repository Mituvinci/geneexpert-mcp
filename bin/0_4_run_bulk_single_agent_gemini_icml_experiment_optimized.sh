...
# Clean up BAM files
if [ -d "$RESULTS_DIR/${DATASET}_single_gemini/stage2_alignment/bam_files" ]; then
  rm -f "$RESULTS_DIR/${DATASET}_single_gemini/stage2_alignment/bam_files/"*.{bam,vcf} 2>/dev/null
fi
echo ""
...
echo "  3. Single Gemini:   $RESULTS_DIR/${DATASET}_single_gemini/"
