...
#   - Fixed output filename to use first BAM basename instead of full path
#   - Sort BAMs by control/treatment keywords (controls first, treatments second)

args = commandArgs(trailingOnly = TRUE)

if (length(args) < 5) {
  stop("Usage: featurecounts.R annotation output_basename control_keyword treatment_keyword [1.bam 2.bam ...]\n  Example: featurecounts.R mm10 DA0036_stroke_vs_control cont ips /path/to/*.bam", call.=FALSE)
}
...
# Run featureCounts on sorted BAM files
cat("\nRunning featureCounts on", length(bam_files_sorted), "BAM files...\n")
fc <- featureCounts(bam_files_sorted, annot.inbuilt=annotation, nthreads=40, isPairedEnd=TRUE)

# EDIT: Clean BAM filenames - remove path and .bam extension
...
# EDIT: Use provided output basename
output_file <- paste0(output_basename, ".count.csv")

# EDIT: Write as CSV instead of space-delimited
